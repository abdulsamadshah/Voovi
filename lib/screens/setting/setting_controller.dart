...
import 'package:voovi/screens/setting/setting_screen.dart';
...
class SettingController extends GetxController {
  ...
  void getInitListData() {
    final bool isLoginCheck = getBoolAsync(SharedPreferenceConst.IS_LOGGED_IN);
    isLoading(true);
  ...
  Future<void> logoutCurrentUser() async {
    ...
    await AuthServiceApis().deviceLogoutApi(deviceId: yourDevice.value.deviceId).then((value) async {
      ...
      Get.find<DashboardController>().addDataOnBottomNav();
      Get.find<DashboardController>().onBottomTabChange(0);
  ...
  Future<void> setPin() async {
    if (newPin.value.isEmpty) {
      ...
      if (newPin.value == confirmPin.value) {
        ...
        isLoading(true);
        await CoreServiceApis().changePin(newPin.value, confirmPin.value).then((value) {
