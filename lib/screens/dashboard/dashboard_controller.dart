...
class DashboardController extends GetxController {
  ...
  void onInit() {
    ...
    if (screen.isEmpty) {
      ...
      screen.addAll(List.generate(screen.length + 1, (_) => const SizedBox()));
    }
    onBottomTabChange(0);
    super.onInit();
  }
  ...
  void addScreenAtPosition(int index, Widget screenWidget) {
    if (screen.length <= index) {
      // Extend the list if the index is out of range
      screen.addAll(List.generate(index - screen.length + 1, (_) => const SizedBox()));
    }
  ...
  Future<void> handleHomeScreen() async {
    final HomeController homeController = getOrPutController(() => HomeController(forceSyncDashboardAPI: true));
    addScreenAtPosition(0, HomeScreen(homeScreenController: homeController));
  }

  Future<void> handleSearchScreen() async {
    final SearchScreenController searchCont = getOrPutController(() => SearchScreenController());
    searchCont.getSearchList();
    addScreenAtPosition(1, SearchScreen(searchCont: searchCont));
  }

  Future<void> handleComingSoonScreen() async {
    final ComingSoonController comingSoonCont = getOrPutController(() => ComingSoonController(getComingSoonList: true));
  ...
  Future<void> handleLiveOrProfileScreen() async {
    if (appConfigs.value.enableLiveTv) {
      final LiveTVController liveTVController = getOrPutController(() => LiveTVController());
      ...
      final ProfileController profileController = getOrPutController(() => ProfileController());
  ...
  Future<void> handleProfileScreen() async {
    if (getBoolAsync(SharedPreferenceConst.IS_LOGGED_IN)) {
      ...
      addScreenAtPosition(4, const ProfileLoginScreen());
    }
  }
