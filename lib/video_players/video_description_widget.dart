...
import 'package:Voovi/screens/subscription/components/subscription_price_component.dart';
import 'package:Voovi/screens/subscription/subscription_controller.dart';
import 'package:Voovi/utils/extension/widget_extention.dart';
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        FittedBox(
          child: Row(
            children: [
              ...
              if (videoDescription.language.isNotEmpty) ...[
              ...
              if (videoDescription.imdbRating.isNotEmpty) ...[
                24.width,
                const CachedImageWidget(
                ...
                6.width,
                Text(
                  "${videoDescription.imdbRating} (${locale.value.imdb})",
        ...
        if ((showWatchNow || isTrailer) && !isFromRentDetails)
          ...
          else if (showWatchNow || videoDescription.isPurchased)
            watchNowButton(
              isTrailer: isTrailer,
              planId: videoDescription.planId,
              requiredPlanLevel: videoDescription.requiredPlanLevel,
              callBack: () async {
                if (videoDescription.movieAccess == MovieAccess.payPerView && videoDescription.isPurchased && videoDescription.purchaseType == PurchaseType.rental) {
        ...
        if (videoDescription.description.isNotEmpty && isDescription) readMoreTextWidget(videoDescription.description).paddingSymmetric(horizontal: 10, vertical: 8),
      ],
    );
  }
}