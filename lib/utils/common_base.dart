...
String movieDurationTime(String time) {
  ...
  final int seconds = parts.length > 2 ? int.parse(parts[2]) : 0;
  ...
  final int s = duration.inSeconds.remainder(60);
...
(double pendingPercentage, String timeLeft) calculatePendingPercentage(String totalDuration, String pendingDuration) {
  ...
  final Duration movieTotalDuration = parseTime(totalDuration);
  final Duration pendingTimeDuration = parseTime(pendingDuration);
  // Calculate the total seconds
  final int totalSeconds = movieTotalDuration.inSeconds;
  final int pendingSeconds = pendingTimeDuration.inSeconds;
  if (totalSeconds <= 0) {
  ...
  final double pendingPercentage = (pendingSeconds / totalSeconds).clamp(0.0, 1.0);
...
String dateFormat(String dateFormat) {
  if (dateFormat.isNotEmpty) {
    final DateTime date = DateTime.parse(dateFormat);
...
String formatMobileNumber(String mobileNumber) {
  ...
  final String countryCode = mobileNumber.substring(0, 2);
  final String firstPart = mobileNumber.substring(2, 4);
  final String lastPart = mobileNumber.substring(mobileNumber.length - 2);
  const String maskedMiddlePart = "******";

  final String formattedNumber = "+$countryCode $firstPart$maskedMiddlePart$lastPart";

  return formattedNumber;
}

DateTime calculateExpirationDate(DateTime startDate, String duration, int durationTime) {
  final int durationTimes = durationTime;
...
String getVideoType({required String type}) {
  ...
  return videoTypeMap[type] ?? VideoType.episode;
}
...
final double screenWidth = WidgetsBinding.instance.window.physicalSize.width /
  ...
  final double screenHeight = WidgetsBinding.instance.window.physicalSize.height /
      WidgetsBinding.instance.window.devicePixelRatio;

  final Offset bottomPosition = Offset(screenWidth / 2, screenHeight - 100); // Centered horizontally, 100px from the bottom
  const Size buttonSize = Size(50, 50);
  if (shareURL.isNotEmpty) {
    Share.shareUri(
      Uri.parse(shareURL.trim()),
     sharePositionOrigin: bottomPosition & buttonSize, 
    );
  } else {
